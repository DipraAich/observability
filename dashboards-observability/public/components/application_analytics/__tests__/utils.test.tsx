...
describe('Utils application analytics helper functions', () => {
  ...
  it('validate removeTabData function', () => {
    const tabIdToBeClosed = 'pomjQYQBg4Jf5lv0c5Ke';
    const newIdToFocus = 'hQ_MOIQB3_rWtxyd0C4T';
    removeTabData(store.dispatch, tabIdToBeClosed, newIdToFocus);
  ...
  it('validate initializeTabData function', () => {
    const tabId = 'application-analytics-tab-hQ_MOIQB3_rWtxyd0C4T';
    const where = 'newTab';
    initializeTabData(store.dispatch, tabId, where);
    const actions = store.getActions();
    const expectedPayload = [
      {
        payload: {
  ...
  it('validate fetchPanelsVizIdList function', () => {
    const appPanelId = 'hA_KOIQB3_rWtxydGS5n';
    httpClientMock.get = jest.fn(() =>
      Promise.resolve((sampleAppAnalyticsPanel as unknown) as HttpResponse)
    );
    expect(fetchPanelsVizIdList(httpClientMock, appPanelId));
  });
});
